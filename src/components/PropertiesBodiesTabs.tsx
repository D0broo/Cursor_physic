...
import {
    ...
    propertiesBodiessData,
    type PropertiesBodiesTabId,
} from "@/data/propertiesBodiesData";
...
function TheoryPanel() {
    return (
      <ol className="space-y-3">
        {propertiesBodiessData.theory.map((item) => (
          <li
            key={item.number}
...
function LawsPanel() {
    return (
      <div className="space-y-4">
        {propertiesBodiessData.laws.map((law) => (
          <article
            key={law.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {propertiesBodiessData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
