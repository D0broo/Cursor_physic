...
import {
    MAGNITNEPOLE_TABS,
    magnitnepoleData,
    type MagnitnepoleTabId,
} from "@/data/magnitnepoleData";
...
function TheoryPanel() {
    return (
      <div className="space-y-4">
        {magnitnepoleData.theory.map((item, index) => (
          item.isHeader ? (
            <h2 
              key={`header-${index}`} 
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {magnitnepoleData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<MagnitnepoleTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel,
};

export default function MagnitnepoleTabs() {
    const [activeTab, setActiveTab] = useState<MagnitnepoleTabId>("theory");
