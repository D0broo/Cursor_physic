...
import {
    POSTIYNIYSTRUM_TABS,
    postiyniyStrumData,
    type PostiyniyStrumTabId,
} from "@/data/postiyniyStrumData";
...
function TheoryPanel() {
    return (
      <ol className="space-y-3">
        {postiyniyStrumData.theory.map((item) => (
          <li
            key={item.number}
...
function LawsPanel() {
    return (
      <div className="space-y-4">
        {postiyniyStrumData.laws.map((law) => (
          <article
            key={law.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {postiyniyStrumData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<PostiyniyStrumTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel,
};

export default function PostiyniyStrumTabs() {
    const [activeTab, setActiveTab] = useState<PostiyniyStrumTabId>("theory");
