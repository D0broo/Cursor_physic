...
import {
    GEOMOPTICA_TABS,
    geomOpticaData,
    type GeomOpticaTabId,
} from "@/data/geomOpticaData";
...
function TheoryPanel() {
    return (
      <div className="space-y-4">
        {geomOpticaData.theory.map((item, index) => (
          item.isHeader ? (
            <h2 
              key={`header-${index}`} 
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {item.imageFile && (
                    <img
                      src={`${BASE_PATH}/images/${item.imageFile}`} // <-- Використовуємо imageFile
                      alt={item.term}
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {geomOpticaData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
function AlgorithmPanel() {
    const { steps, characteristics } = geomOpticaData.algorithm;
    return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {step.imageFile && (
                      <img
                        src={`${BASE_PATH}/images/${step.imageFile}`}
...
const TAB_PANELS: Record<GeomOpticaTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
...
export default function GeomOpticaTabs() {
    const [activeTab, setActiveTab] = useState<GeomOpticaTabId>("theory");
