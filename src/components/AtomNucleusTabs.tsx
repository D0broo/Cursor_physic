...
import {
    ATOMNUCLEUS_TABS,
    atomNucleusData,
    type AtomNucleusTabId,
} from "@/data/atomNucleusData";
...
function TheoryPanel() {
    return (
      <div className="space-y-4">
        {atomNucleusData.theory.map((item, index) => (
          item.isHeader ? (
            <h2 
              key={`header-${index}`} 
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {atomNucleusData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<AtomNucleusTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel
};

export default function AtomNucleusTabs() {
    const [activeTab, setActiveTab] = useState<AtomNucleusTabId>("theory");
