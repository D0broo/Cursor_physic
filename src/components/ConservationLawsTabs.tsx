...
function LawsPanel() {
    return (
      <div className="space-y-4">
        {conservationsData.laws.map((law) => (
          <article
            key={law.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<ConservationsTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel,
};

export default function ConservationsTabs() {
    const [activeTab, setActiveTab] = useState<ConservationsTabId>("theory");
