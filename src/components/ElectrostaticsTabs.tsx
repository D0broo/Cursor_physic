...
import {
    ELECTROSTATICS_TABS,
    electrostaticsData,
    type ElectrostaticsTabId,
} from "@/data/electrostaticsData";
...
function TheoryPanel() {
    return (
      <div className="space-y-4">
        {electrostaticsData.theory.map((item, index) => (
          item.isHeader ? (
            <h2 
              key={`header-${index}`} 
...
function LawsPanel() {
    return (
      <div className="space-y-4">
        {electrostaticsData.laws.map((law) => (
          <article
            key={law.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={1}>
        {electrostaticsData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<ElectrostaticsTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel,
};

export default function ElectrostaticsTabs() {
    const [activeTab, setActiveTab] = useState<ElectrostaticsTabId>("theory");
