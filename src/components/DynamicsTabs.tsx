...
function FormulasPanel() {
  return (
    <ol className="space-y-3" start={35}>
      {dynamicsData.formulas.map((formula) => (
        <li
          key={formula.title}
          className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
        >
...
function AlgorithmsPanel() {
  return (
    <ol className="space-y-3">
      {dynamicsData.algorithms.map((step, index) => (
        <li
          key={step}
          className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
        >
...
const TAB_PANELS: Record<DynamicsTabId, () => ReactNode> = {
  theory: TheoryPanel,
  laws: LawsPanel,
...
export default function DynamicsTabs() {
  const [activeTab, setActiveTab] = useState<DynamicsTabId>("theory");
