...
import {
    WAVEOPTICA_TABS,
    waveOpticaData,
    type WaveOpticaTabId,
} from "@/data/waveOpticaData";
...
function TheoryPanel() {
    return (
      <div className="space-y-4">
        {waveOpticaData.theory.map((item, index) => (
          item.isHeader ? (
            <h2 
              key={`header-${index}`} 
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {waveOpticaData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
export default function WaveOpticaTabs() {
    const [activeTab, setActiveTab] = useState<WaveOpticaTabId>("theory");
