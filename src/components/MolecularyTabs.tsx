...
import {
    MOLECULARY_TABS,
    molecularyData,
    type MolecularyTabId,
} from "@/data/molecularyData";
...
function TheoryPanel() {
    return (
      <ol className="space-y-3">
        {molecularyData.theory.map((item) => (
          <li
            key={item.number}
...
function FormulasPanel() {
    return (
      <ol className="space-y-3" start={35}>
        {molecularyData.formulas.map((formula) => (
          <li
            key={formula.number}
            className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm"
          >
...
const TAB_PANELS: Record<MolecularyTabId, () => ReactNode> = {
    theory: TheoryPanel,
    laws: LawsPanel,
    formulas: FormulasPanel,
};

export default function MolecularyTabs() {
    const [activeTab, setActiveTab] = useState<MolecularyTabId>("theory");
